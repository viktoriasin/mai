...
BEGIN
  LOOP
    INSERT INTO DIM_DATE(DATE_ID, DATE_NAME, "LEVEL")
      values (SEQ_DIM_DATE.NEXTVAL, TO_CHAR(d_date, 'DD.MM.YY'), 1);
    if (EXTRACT(DAY FROM d_date) = 1) then
      INSERT INTO DIM_DATE(DATE_ID, DATE_NAME, "LEVEL")
      values (SEQ_DIM_DATE.NEXTVAL, TO_CHAR(d_date, 'MM.YY'), 2);
      
      if (EXTRACT(MONTH FROM d_date) = 1) then
        INSERT INTO DIM_DATE(DATE_ID, DATE_NAME, "LEVEL")
          values (SEQ_DIM_DATE.NEXTVAL, TO_CHAR(d_date, 'YY'), 3);
      end if;
    end if;
    d_date := d_date + 1;
