...
//  println(persons)
 ...
 // println(personsConcatenated)
  ...
  val personsMutable = mutable.HashSet[Person](
    Person("Ivan", "Ivanov", 40, Sex.Male),
    Person("Petr", "Petrov", 30, Sex.Male),
    Person("Svetlana", "Svetlova", 25, Sex.Female),
    Person("Tatiana", "Kuznetsova", 35, Sex.Female),
...
//  println(persons.filter(p => p.sex == Sex.Female))
  ...
  //Partition - Split xs into a pair of collections; one with elements that satisfy the predicate p, the other with elements that do not, giving the pair of collections (xs filter p, xs.filterNot p)
...
//посчитает средний возраст людей. Функия применяется начиная с начального значения  0 0 ко все элементам коллекции
